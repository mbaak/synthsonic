...
Taken from https://raw.githubusercontent.com/sdv-dev/SDGym/main/sdgym/datasets.py
"""
...
from synthsonic.sdgym.s3 import get_s3_client
...
__all__ = ["load_dataset", "get_available_datasets", "get_dataset_paths"]
...
def _download_dataset(modality, dataset_name, datasets_path=None, bucket=None, aws_key=None, aws_secret=None):
...
def _get_dataset_path(modality, dataset, datasets_path, bucket=None, aws_key=None, aws_secret=None):
    ...
    _download_dataset(modality, dataset, dataset_path, bucket=bucket, aws_key=aws_key, aws_secret=aws_secret)
    return dataset_path
...
def load_dataset(
    modality, dataset, datasets_path=None, bucket=None, aws_key=None, aws_secret=None, limit_dataset_size=None
):
...
def get_dataset_paths(datasets=None, datasets_path=None, bucket=None, aws_key=None, aws_secret=None):
    ...
    if datasets is None:
        ...
        if not is_remote and Path(bucket).exists():
            ...
            datasets = _get_available_datasets("single_table", bucket=bucket)["dataset_name"].tolist()

    return [
        _get_dataset_path("single_table", dataset, datasets_path, bucket, aws_key, aws_secret) for dataset in datasets
    ]
