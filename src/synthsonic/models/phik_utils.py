...
def chi_square(observed, expected, correction=True, lambda_=None):
    ...
    if np.any(observed < 0):
        msg = "All values in `observed` must be nonnegative."
        raise ValueError(msg)
    if observed.size == 0:
        msg = "No data; `observed` has size 0."
        raise ValueError(msg)

    expected = np.asarray(expected)

    terms = np.divide(
        (observed.astype(np.float64) - expected) ** 2, expected, out=np.zeros_like(expected), where=expected != 0
    )
    return np.sum(terms)
...
def phik_matrix(X_obs, X_exp):
    ...
    for i, j in tqdm(itertools.combinations_with_replacement(range(X_obs.shape[1]), 2)):
        if i == j:
            phik_list.append((i, j, 1.0))
            continue
        if n_unique[i] == 1 or n_unique[j] == 1:
            phik_list.append((i, j, 0.0))
            continue
    ...
    return create_correlation_overview_table(phik_list)
